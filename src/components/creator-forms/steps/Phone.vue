<template>
    <div class="phone-style col-xl-6 col-lg-6 col-md-6 col-sm-12 col-12 text-right mr-0 pr-0 pl-0 pl-xl-3 pl-lg-3 pl-md-3 pl-sm-0 pl-0" id="phone">
        <input id="phone_code"  :class="['country-code py-3 mr-0 col-3 block_neutral',{'pl-4' : !country}, {error : phone_code_invalid}]" type="tel" placeholder="+ Код" @change="get_country_code()"  v-model="phone_code_local"/><input id="phone2_number" :class="['block_neutral col-9 pl-4 py-3', {error: phone_invalid }]" type="tel"  placeholder="Доп. телефон (необязательно)" @change="processPhone()" v-model="phone_number_local"/>
    ...
    export default {
      ...
      mounted () {
        if (this.phoneData.code) {
          this.phone_code_local = phoneData.code
        }
        if (this.phoneData.number) {
          this.phone_number_local = this.phoneData.number
        }
      },
      methods: {
        get_country_code: function () {
          this.country = ''
          let code = $('#phone_code').val().trim()
          if (code.indexOf('+') >= 0) {
            code = code.substr(1)
          }
        ...
        processPhone () {
          if (!this.phone_code_invalid) {
            let phoneCode = $('#phone_code').val().trim()
            ...
            if (num === 1) {
              this.$store.commit('CHANGE_REF_PHONE', {
                code: phoneCode,
                number: phoneNumber
              })
            } else {
